...
export const graphSchema = z.object({
  ...
  title: z.string().nullable().optional().describe("Title for the chart"),
  showLegend: z
    .boolean()
    .nullable()
    .optional()
...
export const Graph = React.forwardRef<HTMLDivElement, GraphProps>(
  (
    ...
    const safeClassName = className ?? undefined;
    const safeVariant = variant ?? undefined;
    const safeSize = size ?? undefined;

    return (
      <GraphErrorBoundary className={safeClassName} variant={safeVariant} size={safeSize}>
        <div
          ref={ref}
          className={cn(graphVariants({ variant: safeVariant, size: safeSize }), safeClassName)}
          {...props}
        >
